...
`speckit-tasks`, then `speckit-implement` on branch `feat/$ARGUMENTS` from `main`. For each scenario delegate to the official **`playwright-test-generator`** subagent with the seed `tests/seed.spec.ts` and `project: "seed"` for `generator_setup_page` (it verifies every locator live while generating); then refactor the output to reuse page objects if the generator produced raw locators. Requirements for the final spec:
